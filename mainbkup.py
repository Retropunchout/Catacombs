...
class Option:
    def __init__(self, description, tags, boost_tag, negative_tag, check, check_num, reward, failure):
        self.description = description
        self.tags = tags
...
class Combat:
    ...
    def cast_spell(self, attacker, victim):
        spell_name_list = ["darkness seethes towards the {} from the {}, causing {} damage", "tendrils of madness rip forth from the {}, hitting the {} for {} damage"]
        crit_string = ''
        # roll to hit
        hit_roll = random.randint(1, 6)
...
def init():
    ...
    with open('encounters.json') as enc_json:
        ...
        for enc in encs["encounters"]:
            ...
            encounter.options = []
            for opt in options:
                option = Option(description=opt["description"],tags=opt["tags"],boost_tag=opt["boost_tag"],negative_tag=opt["negative_tag"],check=opt["check"],check_num=opt["check_num"], reward=opt["reward"], failure=opt["failure"])
    ...
    for deity in session_world.deities:
        pray_option=Option("Pray to {}".format(deity.name),tags=[],boost_tag=[],negative_tag=[],check="intelligence",check_num=1,reward={"blessing":"You receive a blessing from {}".format(deity.name)},failure={"curse":"{} curses you!".format(deity.name)})
        leave_option = Option("Leave", tags=[], boost_tag=[], negative_tag=[], check="none", check_num=1,
    ...
    with open('items.json') as enc_json:
        ...
        for ite in items["items"]:
            item = Item(name=ite["name"], item_type=ite["item_type"], tags=ite["tags"], damage=ite["dmg"], armour=ite["armour"])
    ...
    #start first area
    session_world.current_area = Area(name="Crumbling ruins",area_type="cave", description="A cave network filled with crumbling ruins",levels=10,tags=[])
    session_world.current_encounter = random.choice(session_world.encounters.values())
...
class Game(BoxLayout):
    ...
    def next_area(self):
        ...
        session_world.current_area = new_area
        session_world.current_level = 0

    def next_level(self, emitter):
        if session_world.current_level > session_world.current_area.levels:
            self.next_area()
        else:
    ...
    def end_combat(self, monster):
        ...
        holder_option = Option(description="",tags=[],boost_tag=[],negative_tag=[],check="none",check_num=0,reward={},failure={})
        session_world.current_encounter = Encounter(name="combat",description=end_text,tags=[],item_tags=[],options=[holder_option])

        self.update_all('')
        self.move_option()

        #self.encounter_text.text = end_text

    def loot(self, monster):
        if "creature" not in monster.tags:
    ...
    def create_combat(self):
        ...
        def light_attack(emitter):
            ...
            self.msgs += "\n" + msg_return
            status_str = combat.status_check(player=Player, monster=selected_monster)
            if status_str == "monster_dead":
                self.view.dismiss()
        ...
        self.log = AutoScrollableVLabel(text=self.msgs, pos_hint={"right":1.2})
    ...
    def get_char_string(self, char):
        print getattr(Player,'dexterity')
